...
class Enemy(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.top > screen_height:
            ...
            self.rect.top = 0
            self.rect.center = (random.randint(40, screen_width - 40), -20)
            if score % 5 == 0:  # добавлено: увеличение скорости каждые 5 очков
                speed += 1
...
class Coin(pygame.sprite.Sprite):
    def __init__(self, enemy):
        ...
        self.weight = random.randint(1, 5)  # добавлено: случайный вес монеты
    def move(self, enemy):
        self.rect.move_ip(0, speed)
        if self.rect.top > screen_height:
            self.rect.top = 0
...
while loop:
    ...
    if pygame.sprite.spritecollide(P1, coins_group, dokill=True):
        ...
        coin_score += coin.weight  # добавлено: увеличение очков в зависимости от веса монеты
        coin = Coin(E1)
        coins_group.add(coin)
